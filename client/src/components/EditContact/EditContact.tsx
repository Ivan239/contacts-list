...
import styles from './EditContact.module.sass';
...
function EditContact({ setShowEditContact, name, phone, cid }: EditContactProps) {
  const {
    register,
    handleSubmit,
    formState: { errors },
  ...
  const onSubmit = async (data: { [x: string]: string }): Promise<void> => {
    ...
    axios
      .put(`${serverhost}/data/${uid}/${cid}`, JSON.stringify(newAccount), {
        headers: {
      ...
      .then(() => {
        store.dispatch(editContact(newAccount));
  ...
  return (
    <div className={styles.modal} onClick={() => setShowEditContact(false)}>
      <div className={styles.editcontact} onClick={(e) => e.stopPropagation()}>
        <img
          src={close}
          alt="close"
